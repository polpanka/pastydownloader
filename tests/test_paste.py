...
# ytDlpStorageDir) risolve a una cartella basata sull'argv0 del test runner invece
# che su quella vera dell'app - stesso setup del vero main.py
...
def makePastylinkUrl(realUrl, extra=None):
    payload = {'v1': {'ytdlp': realUrl}, **(extra or {})}
...
class _FakeStatusQueue:
    ...
    def showNow(self, text, seconds=0):
        self.shownNow.append((text, seconds))
...
class _FakeFullAppParent(FakeAppParent):
    ...
    def __init__(self, grid):
        ...
        self.isOnline = True
        self.dependenciesReady = True  # simula ffmpeg/yt-dlp gia' scaricati: non e' l'oggetto di questi test
        self.button = _FakeWidget()
        self.menu = _FakeMenu()
        self.statusBar = _FakeStatusBar()
        self.statusQueue = _FakeStatusQueue()
    ...
    def isUiUnlocked(self):
        return main.Pasty.isUiUnlocked(self)

    def _refreshLockState(self):
        return main.Pasty._refreshLockState(self)
...
class GetReferersResilientToFetchFailureTest(unittest.TestCase):
    """Pasty.getReferers() non deve propagare un fallimento nello scaricare
    referer.json (rete assente, url spostato, risposta non json...): deve
    degradare a 'nessun referer noto' invece di far esplodere il chiamante,
    e non e' vitale: il fallimento va ricordato per il resto della sessione,
    non ritentato ad ogni singola riga del batch."""

    def test_returns_empty_dict_and_caches_the_failure_instead_of_retrying(self):
        class _Bare:
            referers = None
        obj = _Bare()
        with mock.patch.object(Tools, 'readFileJson', side_effect=ConnectionError('rete assente')) as mockFetch:
            result = main.Pasty.getReferers(obj)
            self.assertEqual(result, {})
            self.assertEqual(obj.referers, {})  # fallimento messo in cache
            main.Pasty.getReferers(obj)  # una seconda chiamata...
        mockFetch.assert_called_once()  # ...non deve ritentare il download
...
class PasteMp4Test(unittest.TestCase):
    ...
    def test_mp4_url_is_routed_to_direct_video_engine(self):
        ...
        pastedUrl = PastedUrl(self.app, 0, MP4_URL, Tools.checkFFmpeg())
...
class PasteM3u8ManifestTest(unittest.TestCase):
    ...
    def test_manifest_row_is_written_to_file_and_routed_to_direct_video_engine(self):
        ...
        pastedUrl = PastedUrl(self.app, 0, rawManifestFromGrid, Tools.checkFFmpeg())
...
class PasteYoutubeTest(unittest.TestCase):
    ...
    def test_plain_youtube_url_still_reaches_ytdlp_via_fallback(self):
        ...
        with mock.patch.object(PastedUrl, '_getContentTypeFromUrl', return_value='text/html'), \
            ...
            pastedUrl = PastedUrl(self.app, 0, YOUTUBE_URL, Tools.checkFFmpeg())
...
class PastylinkUrlTest(unittest.TestCase):
    ...
    'httpasty://<base64 di un json>' e' generato dal sito pasty.link stesso
    ...
    resta l'url httpasty:// grezzo finche' non viene costruito un PastedUrl
    (al momento dello scaricamento) - a quel punto PastedUrl sostituisce
    subito la cella con l'url vero decodificato (vedi PastedUrl.__init__)."""

    def setUp(self):
        self.app = FakeAppParent(makeGrid())

    def test_is_pastylink_url(self):
        self.assertTrue(Tools.isPastylinkUrl('httpasty://abc'))
    ...
    def test_decode_invalid_payload_returns_none(self):
        self.assertIsNone(Tools.decodePastylinkUrl('httpasty://non-e-base64-valido!!!'))

    def test_decode_missing_v1_key_returns_none(self):
        encoded = base64.b64encode(json.dumps({'title': 'senza v1'}).encode()).decode().rstrip('=')
        self.assertIsNone(Tools.decodePastylinkUrl(Tools.PASTYLINK_URL_PREFIX + encoded))

    def test_decode_v1_without_ytdlp_key_returns_none(self):
    ...
    def test_paste_pastylink_url_is_added_to_grid_raw(self):
        ...
        with mock.patch.object(Tools, 'pasteFromClipboard', return_value=pastylinkUrl):
            main.Pasty.pasteUrls(self.app)
        # in griglia resta l'url httpasty:// grezzo, non ancora decodificato
    ...
    def test_pastylink_url_is_decoded_and_routed_to_ytdlp_when_pastedurl_is_built(self):
        """Simula cosa fa AsyncWorker.runAllUrls al momento dello scaricamento."""
        pastylinkUrl = makePastylinkUrl(YOUTUBE_URL)
        self.app.pastyGrid.addRow(pastylinkUrl)  # riga vera: PastedUrl vi scrive sopra l'url decodificato
        pastedUrl = PastedUrl(self.app, 0, pastylinkUrl, Tools.checkFFmpeg())  # nessun mock: isPastylinkUrl e' un puro controllo di stringa, zero rete
    ...
    def test_pastylink_url_is_replaced_in_the_grid_once_decoded(self):
        # cosi' un "Copia link"/ri-download successivo usa l'url vero, non
        # piu' il blob httpasty:// grezzo incollato all'inizio
        pastylinkUrl = makePastylinkUrl(YOUTUBE_URL)
        self.app.pastyGrid.addRow(pastylinkUrl)
        PastedUrl(self.app, 0, pastylinkUrl, Tools.checkFFmpeg())
        self.assertEqual(self.app.pastyGrid.getCellUrl(0), YOUTUBE_URL)

    def test_invalid_pastylink_url_is_not_written_into_the_grid(self):
        # il decode e' fallito (isPastylink resta False): la cella deve
        # restare quella grezza, non venire sovrascritta con spazzatura
        invalidUrl = 'httpasty://non-valido!!!'
        self.app.pastyGrid.addRow(invalidUrl)
        PastedUrl(self.app, 0, invalidUrl, Tools.checkFFmpeg())
        self.assertEqual(self.app.pastyGrid.getCellUrl(0), invalidUrl)
    ...
    def test_invalid_pastylink_url_built_directly_falls_back_to_raw_url(self):
        ...
        invalidUrl = 'httpasty://non-valido!!!'
        pastedUrl = PastedUrl(self.app, 0, invalidUrl, Tools.checkFFmpeg())
...
class UiLockTest(unittest.TestCase):
    """isUiUnlocked/onFfmpegReady/onYtDlpReady: ffmpeg e yt-dlp non sono piu'
    imbarcati ma scaricati IN CASCATA al primo avvio (prima ffmpeg, poi solo
    se e' andato a buon fine yt-dlp - vedi ffmpeg_installer.py,
    ytdlp_updater.py, Pasty.initDependencies in main.py),
    e l'interfaccia deve restare bloccata (stessa modalita' TYPE_NO_INTERNET
    gia' usata per l'offline) finche' connessione e dependenciesReady non
    sono entrambi a posto."""

    def setUp(self):
        self.app = _FakeFullAppParent(makeGrid())

    def test_ui_locked_when_dependencies_not_ready_even_if_online(self):
        self.app.isOnline = True
        self.app.dependenciesReady = False
        self.assertFalse(main.Pasty.isUiUnlocked(self.app))

    def test_ui_locked_when_offline_even_if_dependencies_ready(self):
        self.app.isOnline = False
        self.app.dependenciesReady = True
        self.assertFalse(main.Pasty.isUiUnlocked(self.app))

    def test_ui_unlocked_only_when_both_are_true(self):
        self.app.isOnline = True
        self.app.dependenciesReady = True
        self.assertTrue(main.Pasty.isUiUnlocked(self.app))

    def test_on_ffmpeg_ready_failure_shows_popup(self):
        self.app.dependenciesReady = False
        with mock.patch.object(self.app, 'showInstallFailedPopup') as popup:
            main.Pasty.onFfmpegReady(self.app, False)
        self.assertFalse(self.app.dependenciesReady)
        popup.assert_called_once_with()

    def test_on_ffmpeg_ready_success_starts_ytdlp_download_without_unlocking_yet(self):
        # ffmpeg pronto, ma la cascata non e' completa: deve partire il
        # download di yt-dlp, la UI deve restare bloccata e non deve ancora
        # comparire "tutto pronto"
        self.app.dependenciesReady = False
        self.app.ytDlpUpdater = mock.Mock()
        with mock.patch('main.threading.Thread') as ThreadClass:
            main.Pasty.onFfmpegReady(self.app, True)
        ThreadClass.assert_called_once_with(target=self.app.ytDlpUpdater.ensureInstalled, daemon=True)
        ThreadClass.return_value.start.assert_called_once()
        self.assertFalse(self.app.dependenciesReady)
        self.assertEqual(self.app.statusQueue.shownNow, [])

    def test_on_ytdlp_ready_failure_shows_popup(self):
        self.app.dependenciesReady = False
        with mock.patch.object(self.app, 'showInstallFailedPopup') as popup:
            main.Pasty.onYtDlpReady(self.app, False)
        self.assertFalse(self.app.dependenciesReady)
        popup.assert_called_once_with()

    def test_on_ytdlp_ready_success_completes_the_cascade_and_shows_done_message(self):
        self.app.isOnline = True
        self.app.dependenciesReady = False
        with mock.patch.object(self.app.pastyGrid, 'setContextMenuType'), \
             mock.patch.object(self.app, 'startYtDlpPeriodicUpdates', create=True):
            main.Pasty.onYtDlpReady(self.app, True)
        self.assertTrue(self.app.dependenciesReady)
        self.assertEqual(self.app.statusQueue.shownNow, [(MyText().setupCompleteMsg, 5)])


class InstallFailedPopupTest(unittest.TestCase):
    """showInstallFailedPopup: un solo bottone di conferma, poi l'app si
    chiude sempre - per riprovare bisogna rilanciare l'eseguibile."""

    def _fakeApp(self):
        return mock.Mock()

    def _patchedBox(self):
        fakeBox = mock.Mock()
        fakeBox.addButton.return_value = mock.Mock(name='close')
        return fakeBox

    def test_closes_the_app_after_the_popup_is_dismissed(self):
        fakeApp = self._fakeApp()
        with mock.patch('main.QMessageBox', return_value=self._patchedBox()):
            main.Pasty.showInstallFailedPopup(fakeApp)
        fakeApp.close.assert_called_once()

    def test_shows_the_generic_message(self):
        fakeApp = self._fakeApp()
        with mock.patch('main.QMessageBox', return_value=self._patchedBox()) as MessageBoxClass:
            main.Pasty.showInstallFailedPopup(fakeApp)
        MessageBoxClass.return_value.setText.assert_called_once_with(MyText().installFailedMsg)
...
class PasteGenericMediaTest(unittest.TestCase):
    ...
    def test_media_is_classified_as_generic(self):
        ...
        for name, (url, ext, _magic) in self.CASES.items():
            with self.subTest(media=name):
                pastedUrl = PastedUrl(self.app, 0, url, ffmpeg)
...
class PasteGenericYtDlpFallbackTest(unittest.TestCase):
    ...
    un url httpasty://, ma nasconde un video imbarcato: come penultima
...
class PasteMultiTrackM3u8Test(unittest.TestCase):
    ...
    def test_master_playlist_with_separate_audio_group_is_routed_to_direct_video_engine(self):
        pastedUrl = PastedUrl(FakeAppParent(makeGrid()), 0, MULTI_TRACK_M3U8_URL, Tools.checkFFmpeg())
        if pastedUrl.getEngine() == PastedUrl.URL_TYPE_GENERIC:
...
class ContentTypeClassificationTest(unittest.TestCase):
    ...
    def classify(self, contentType):
        with mock.patch.object(PastedUrl, '_getContentTypeFromUrl', return_value=contentType):
            return PastedUrl(self.app, 0, 'https://example.test/resource', Tools.checkFFmpeg())
...
class RunAllUrlsSurvivesConstructionErrorsTest(unittest.TestCase):
    """Se costruire PastedUrl solleva un'eccezione per una riga (es.
    referer.json irraggiungibile durante l'analisi, vedi PastedUrl._analyze),
    runAllUrls() non deve morire in silenzio: gira in un threading.Thread
    grezzo (vedi Pasty.moveProcessInSeparatedThread), quindi un'eccezione
    non gestita lascerebbe l'interfaccia bloccata per sempre in 'Attendere...'
    (allProcessesFinished(), che emette il segnale che sblocca la UI, non
    verrebbe mai raggiunto). La riga va invece segnata come errore e il
    batch deve proseguire fino in fondo."""

    def setUp(self):
        self.grid = makeGrid()
        self.app = FakeAppParent(self.grid)

    def test_construction_error_on_one_row_does_not_block_the_batch(self):
        self.grid.addRow(MP4_URL)
        self.grid.setCellConvert(0, self.app.ACTION_TO_MP4)
        worker = AsyncWorker(self.app, [0], Tools.checkFFmpeg())
        with mock.patch('worker.PastedUrl', side_effect=RuntimeError('referer.json irraggiungibile')), \
             mock.patch.object(AsyncWorker, 'allProcessesFinished') as mockFinished:
